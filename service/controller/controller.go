...
func (c *Controller) Start() error {
	...
	// 初始化URL记录器
	if c.config.URLLoggerConfig != nil {
		urlLogger := urllogger.New(c.config.URLLoggerConfig, c.clientInfo.NodeID, c.apiClient)
		if err := urlLogger.Start(); err != nil {
			c.logger.WithError(err).Error("启动URL记录器失败")
		} else {
			// 将URL记录器设置到dispatcher中
			c.dispatcher.URLLogger = urlLogger
			c.logger.Info("URL记录器已启动")
		}
	}
...
func (c *Controller) Close() error {
	...
	// 停止URL记录器
	if c.dispatcher != nil && c.dispatcher.URLLogger != nil {
		if err := c.dispatcher.URLLogger.Stop(); err != nil {
			c.logger.WithError(err).Error("停止URL记录器失败")
		} else {
			c.logger.Info("URL记录器已停止")
		}
	}
