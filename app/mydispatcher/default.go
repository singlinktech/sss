...
type DefaultDispatcher struct {
	...
	URLLogger   *urllogger.URLLogger // 添加URL记录器
}
	...
	if sessionInbound.User != nil {
		...
		if d.URLLogger != nil {
			...
			if content := session.ContentFromContext(ctx); content != nil {
				...
				// 构建额外信息
				extraInfo = fmt.Sprintf("level:%d,tag:%s,network:%s",
					sessionInbound.User.Level,
					sessionInbound.Tag,
			...
			if destination.Address.Family().IsDomain() {
				...
				fullURL = fmt.Sprintf("%s://%s:%d", protocol, domain, destination.Port)
			} else {
				// 如果是IP地址，尝试通过其他方式获取域名
